...
startButton.addEventListener("click", async () => {
    ...
    mediaRecorder.onstop = () => {
        ...
        stopButton.style.background = "rgba(241, 207, 81, 1)";
    ...
    // 按钮样式与功能（写死）
    startButton.style.background = "rgba(241, 207, 81, 1)";
    ...
    try {
        ...
        reader.onload = async function(event) {
            try {
                ...
                // 将文章ID添加到 FormData 中
                const articleId = localStorage.getItem("courseContent");
                if (articleId) {
                    formData.append('courseId', articleId);
                } else {
                    throw new Error('courseContent 未找到，请确认缓存中是否存在 courseContent 数据。');
                }
